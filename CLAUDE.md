...
## Reading a running game's memory

`apps/gba/src/symbols.js` is generated by `tools/gen-symbols.mjs` from pret's
published symbol files, and committed. Before it, every address the emulator
app read was found by searching a running machine's RAM for the shape it must
have. That works — and one of the five it produced was wrong for weeks:
`0x0203b0a8`, believed to be "which Pokémon is out in battle", is nine bytes
into `gPartyMenu` and reads zero in an ordinary battle, so it was right for the
wrong reason and never failed loudly. A name from the decompilation is better
evidence than a shape that happens to fit.

Three rules make this safe rather than a table of magic numbers. The generator
reads **both published builds** and refuses to emit anything whose RAM address
differs between them, so "FireRed and LeafGreen share a layout" is checked
rather than assumed. Code addresses *do* differ, by twenty bytes, so they ship
as lists and the running one is matched with `.includes` — the match is its own
proof, and a build in neither list falls back to the behaviour that existed
before. And the shape test that originally found the party still runs on every
read, because a record caught mid-write is not a wrong address but looks like
one.

Each new address earns its place by removing a guess. `sLockFieldControls`
replaced "press a direction for two seconds and if the tile never changed,
assume a message box" — it cannot tell a box from a wall, and answering a wall
with A talks to whoever is behind it. `gBattleTypeFlags` replaced nothing at
all: a trainer cannot be fled, the game says so, and without that the walker
pressed RUN at the Nugget Bridge for a full minute and then reported that the
battle had stopped responding.
...
expensive ones: `leadWith` lived there, worked there, and was never ported, so
the app booked a trip to the Pokémon Center after every battle for a week and
nothing that ran headless could see it. A harness that does not run the shipped
code proves nothing about the shipped code.
